/*-----------------------------------------------------------------------------
 *作者:毛伟 , email:cugbmao@163.com
 *http://cugbmao09.iteye.com/
 *version:1.0  , 时间：2012-12-20
 *desktop需引入Winter.js创建命名空间，和jquery 及 jquery ui,contextMenu jquery插件
-----------------------------------------------------------------------------*/
//声明desktop空间,封装相关操作
Winter.NS("desktop");
Winter.desktop = {
	...
	contextMenu : function(jqElem, data, menuName, textLimit) {
		...
		jqElem.smartMenu(data, {
			...
			afterShow : function() {
				...
				var myData = Winter.desktop.getMydata(), wh = myData.winWh;//获取当前document宽高
				var menuXY = menu.offset(), menuH = menu.height(), menuW = menu
						.width();
...
Winter.NS("desktop.win");
Winter.desktop.win = {
	winHtml : function(title, url, id) {
		return url.indexOf('#')==0?("<div class='windows corner' id="
				+ id
				+ "><div class='win_title'><b>"
				+ title
				+ "</b><span class='win_btnblock'><a href='#' class='winMinimize' title='最小化'></a><a href='#' class='winMaximize' title='最大化'></a><a href='#' class='winHyimize' title='还原'></a><a href='#' class='winClose' title='关闭'></a></span></div>"
				+ $(url).html()
				+ "</div>"):("<div class='windows corner' id="
				+ id
				+ "><div class='win_title'><b>"
				+ title
		...
		obj
				.each(function() {
					...
					if (o.find('.zzDiv').size() <= 0)
						o
								.append($("<div class='zzDiv' style='width:100%;height:"
										+ (o.innerHeight() - 26)
	...
	closeWin : function(obj) {
		...
		Winter.desktop.taskBar.delWinTab(obj);
	...
	minimize : function(obj) {
		...
		if ($win.size() > 1) {
			...
			objTab = Winter.desktop.taskBar.findWinTab(obj);
			objTab.removeClass('selectTab').addClass('defaultTab');
		}
	},
	//最大化窗口函数	
	maximizeWin : function(obj) {
		var myData = Winter.desktop.getMydata(), panel = $("#desktopInnerPanel")
	...
	switchZindex : function(obj) {
		var myData = Winter.desktop.getMydata(), $topWin = myData.topWin, $topWinTab = myData.topWinTab, curWinZindex = obj
				.css("z-index"), maxZ = myData.maxZindex, objTab = Winter.desktop.taskBar
				.findWinTab(obj);
	...
	newWin : function(options) {
		...
		var myData = Winter.desktop.getMydata(), wh = myData.winWh, //获取当前document宽高
		$windows = $("div.windows"), _this = this, curwinNum = Winter._is(
		...
		var wLeft = Winter._is(options['WindowPositionLeft'], "Number") ? options['WindowPositionLeft']
				+ dxy - panelLeft.left
				: (wh['w'] - options['WindowWidth']) / 2 + dxy - panelLeft.left;
		var wTop = Winter._is(options['WindowPositionTop'], "Number") ? options['WindowPositionTop']
				+ dxy / 2
		...
		if ($("#" + id).size() <= 0) {
			//在任务栏里添加tab
			Winter.desktop.taskBar.addWinTab(options['WindowTitle'],
					options['WindowsId']);
			//初始化新窗口并显示
			$(_this.winHtml(options['WindowTitle'], options['iframSrc'], id))
					.appendTo('#desktopInnerPanel');

			var $newWin = $("#" + id), $icon = $("#" + options['WindowsId']), $iconOffset = $icon
					.offset(), $fram = $newWin.find(".winframe"), $winTitle = $newWin
					.find(".win_title"), winMaximize_btn = $newWin
					.find('a.winMaximize')//最大化按钮
			...
			//如果已存在此窗口，判断是否隐藏
			var wins = $("#" + id), objTab = Winter.desktop.taskBar
					.findWinTab(wins);
			if (wins.is(":hidden")) {
				wins.show();
				objTab.removeClass('defaultTab').addClass('selectTab');//当只有一个窗口时
				Winter.desktop.win.switchZindex(wins);
			} else {
...
//侧边工具栏
Winter.NS("desktop.lrBar");
Winter.desktop.lrBar = {
	upLrBar : function() {
		var myData = Winter.desktop.getMydata(), $lrBar = myData.panel.lrBar['_this'], wh = myData.winWh;
		$lrBar.css({
	...
	init : function(iconData) {
		//读取元素对象数据
		var myData = Winter.desktop.getMydata(), $default_tools = myData.panel.lrBar['default_tools'], $def_tools_Btn = $default_tools
		...
		//附加data数据
		Winter.desktop.iconDataInit(iconData);

		//如果窗口大小改变，则更新侧边栏位置
		$(window).wresize(function() {
			Winter.desktop.winWH();//更新窗口大小数据			   
			_this.upLrBar();
		});
...
/*----------------------------------------------------------------------------------	
 //声明任务栏空间，任务栏相关js操作
 ----------------------------------------------------------------------------------*/
Winter.NS("desktop.taskBar");
Winter.desktop.taskBar = {
	timer : function(obj) {
	...
	upTaskWidth : function() {
		var myData = Winter.desktop.getMydata(), $task_bar = myData.panel.taskBar['_this'];
	...
	init : function() {
		//读取元素对象数据
		var myData = Winter.desktop.getMydata();
		...
		var data = [
				[
						...
						},
						{
							text : "最小化",
							func : function() {
								Winter.desktop.win.minimize($("#myWin_"
										+ tab.data('win')));
							}
						} ],
				[ {
					...
					func : function() {
						...
						Winter.desktop.win.closeWin($("#myWin_"
								+ tab.data('win')));
					}
				} ] ];
		Winter.desktop.contextMenu(tab, data, "taskTab_menu" + id, 10);
	},
							...
							if (desk.is(".currDesktop")) {
								if (win.is(".hideWin")) {
									...
									$(this).removeClass('defaultTab').addClass(
											'selectTab');//当只有一个窗口时
									Winter.desktop.win.switchZindex(win);
								} else {
									if ($(this).hasClass('selectTab')) {
										Winter.desktop.win.minimize(win);
									} else {
										Winter.desktop.win.switchZindex(win);
									}
								}
	...
	delWinTab : function(wObj) {
		var myData = Winter.desktop.getMydata(), $task_lb = myData.panel.taskBar['task_lb'], $task_bar = myData.panel.taskBar['_this'], LeftBtn = $('#leftBtn'), rightBtn = $('#rightBtn'), pos = $task_lb
				.position();
	...
	findWinTab : function(wObj) {
		var myData = Winter.desktop.getMydata(), $task_lb = myData.panel.taskBar['task_lb'], objTab;
...
//navbar
Winter.NS("desktop.navBar");
Winter.desktop.navBar = {
	init : function() {
		var myData = Winter.desktop.getMydata(), $navBar = myData.panel.navBar, $innerPanel = myData.panel.desktopPanel.innerPanel, $navTab = $navBar
		...
		$navTab.droppable(
				...
				function(event) {
					...
					event.stopPropagation();
					var i = $navTab.index($(this));
					Winter.desktop.deskIcon.desktopMove($innerPanel, $deskIcon,
...
//桌面背景
Winter.NS("desktop.wallpaper");
Winter.desktop.wallpaper = {
	init : function(imgUrl) {

		//将当前窗口宽度和高度数据存储在body元素上
		Winter.desktop.winWH();

		var myData = Winter.desktop.getMydata(), winWh = myData.winWh, wallpaper = myData.panel.wallpaper, _this = this;

		if (imgUrl !== null) {
			wallpaper.html("<img src='" + imgUrl + "'></img>");
			var img = wallpaper.find("img");

			Winter.getImgWh(imgUrl, function(imgW, imgH) {
				if (imgW <= winWh.w) {
...
//桌面图标区域
Winter.NS("desktop.deskIcon");
Winter.desktop.deskIcon = {
	addIcon : function(data) {
		var _this = this;
		if (!$("#" + data.id).size()) {
			var iconHtml = "<li class='desktop_icon' id='" + data.id
					+ "'><span class='icon'><img src='" + data.iconSrc
					+ "'/></span><div class='text'>" + data.title
					+ "<s></s></div></li>";
	...
	arrangeIcons : function(desktop) {
		var myData = Winter.desktop.getMydata(), winWh = myData.winWh, $navBar = myData.panel.navBar, navBarHeight = $navBar
				.outerHeight()
		//计算一共有多少图标
	...
	upDesktop : function($deskIcon, $deskIconBlock, $innerPanel, $deskIconNum,
		...
		var myData = Winter.desktop.getMydata(), winWh = myData.winWh, w = winWh['w']
				- lBarWidth, h = (winWh['h'] - 75 - navBarHeight), _this = this;
	...
	init : function(iconData) {
		...
		var myData = Winter.desktop.getMydata(), winWh = myData.winWh, $deskIconBlock = myData.panel.desktopPanel['_this'], $innerPanel = myData.panel.desktopPanel.innerPanel, $deskIcon = myData.panel.desktopPanel['deskIcon'], $deskIconNum = $deskIcon
				.size(), $navBar = myData.panel.navBar, navBarHeight = $navBar
				.outerHeight(), $navTab = $navBar.find("a"), lBarWidth = myData.panel.lrBar["_this"]
				.outerWidth(), _this = this;

		_this.upDesktop($deskIcon, $deskIconBlock, $innerPanel, $deskIconNum,
				navBarHeight, lBarWidth);
		...
		//单击添加应用
		$deskIcon.find("li.add_icon").click(function() {
		...
		Winter.desktop.contextMenu($(document.body), data, "body", 10);
	}
}
